...
function searchResultsMarkup(q, data) {
  ...
  return `<div class="search-results-heading"><h2>搜索结果</h2><p class="search-summary">找到 ${data.pagination.total} 条与“${escapeHtml(q)}”相关内容</p></div><div class="${uiClass("card search-results")}">${data.items.map((item)=>{ const label = searchResultTypeLabel(item.type); const href = item.type === "article" ? routePath("guide", { slug: item.slug }) : item.type === "tool" ? (item.href || routePath("tool", { tool: item.slug })) : routePath("wikiEntry", { datasetSlug: item.dataset_slug, entrySlug: item.slug }); return `<a class="row-result" href="${href}" aria-label="查看${escapeHtml(item.title)}">${label ? `<span class="row-result-type">${escapeHtml(label)}</span>` : ""}<h3>${highlightSearchText(item.title, q)}</h3><p>${highlightSearchText(item.snippet, q)}</p></a>`; }).join("<hr>")}</div>`;
}
